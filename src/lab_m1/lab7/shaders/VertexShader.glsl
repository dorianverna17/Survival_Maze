...
void main()
{
    ...
    vec3 world_normal = vec3 (mat3(Model) * normalize(v_normal)); //vec4 (v_position, 0));
    ...
    float d = distance(v_position, light_position);
    float Kc = 1;
    float Kl = 0.2;
    float Kq = 0.1;
    float factorAtenuare = 1 / (Kc + Kl * d + Kq * d * d);
    ...
    color = intensitate * object_color;
